...
use Exception;
use Cncal\Alipay\Sdk\Aop\AopEncrypt;
use Cncal\Alipay\Sdk\Aop\SignData;
use Cncal\Alipay\Sdk\Aop\EncryptParseItem;
...
class AopClient
{
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt)
    {
		...
		$logData = array(
			...
			'ip' => $localIp,
			'os' => PHP_OS,
			'sdk_version' => $this->alipaySdkVersion,
			'request_url' => $requestUrl,
			'error_code' => $errorCode,
			str_replace("\n", "", $responseTxt)
		);

        file_put_contents(storage_path('logs/alipay.log'), date("Y-m-d H:i:s")."  ".$logData."\r\n", FILE_APPEND);
	}
     ...
     * @return \Cncal\Alipay\Sdk\Aop\提交表单HTML文本|string
     * @throws \Exception
     */
	 ...
	 * 公钥是否是读取字符串还是读取文件，是根据初始化传入的值判断的。
	 */
	public function checkSignAndDecrypt($params, $rsaPublicKeyPem, $rsaPrivateKeyPem,
                                        $isCheckSign, $isDecrypt, $signType = 'RSA')
    {
	 ...
	 * 公钥是否是读取字符串还是读取文件，是根据初始化传入的值判断的。
	 */
	public function encryptAndSign($bizContent, $rsaPublicKeyPem, $rsaPrivateKeyPem,
                                   $charset, $isEncrypt, $isSign, $signType = 'RSA')
    {
		// 加密，并签名
		if ($isEncrypt && $isSign) {
     ...
     * @param $responseContent
     * @param $responseJSON
     * @return \Cncal\Alipay\Sdk\Aop\SignData
     */
    ...
    {
		if (!$this->checkEmpty($this->alipayPublicKey) ||
            !$this->checkEmpty($this->alipayrsaPublicKey)) {
			if ($signData == null ||
                $this->checkEmpty($signData->sign) ||
			...
			if (!$this->checkEmpty($responseSubCode) ||
				...
				$checkResult = $this->verify(
				    $signData->signSourceData, $signData->sign,
     ...
     * @param $request
     * @param $responseContent
     * @return \Cncal\Alipay\Sdk\Aop\EncryptParseItem|null
     */
     ...
     * @param $nodeName
     * @param $nodeIndex
     * @return \Cncal\Alipay\Sdk\Aop\EncryptParseItem
     */
     ...
     * @return string
     */
	function camelize($words, $separator)
    {
